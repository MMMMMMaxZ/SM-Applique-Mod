...
alertCreator.colorNormal = sm.color.new(0x114514ff) 
...
function alertCreator.server_onCreate( self )
	...
	--self.lastCreationID = self.shape.body:getCreationId()
	self.firstLift = false
...
---------------新的判定方法(针对焊接问题)：当creationID改变时开始判断（改变条件：搭砖块 / 放上架子---->不用像原本读取作品所有shape后再对比数量了）
---------------新的判定方法：在第一次放上架子时，数数量，大于3直接自爆，简单粗暴，没有漏洞
	if self.shape.body:isOnLift() then
		if not self.firstLift then
			--self.lastCreationID = self.shape.body:getCreationId()
			self.firstLift = true
			-----check amount >3?
			local creationShapes = self.shape.body.getCreationShapes(self.shape.body)
	...
	--[[if self.shape.body:getCreationId() ~= self.lastCreationID then
		self.lastCreationID = self.shape.body:getCreationId()
		print("1")
	end]]--
