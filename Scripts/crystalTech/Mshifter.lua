--Mshifter
Mstr = class(nil)
Mstr.maxParentCount = -1
Mstr.connectionInput = sm.interactable.connectionType.logic
Mstr.maxChildCount = -1
Mstr.connectionOutput = sm.interactable.connectionType.logic+sm.interactable.connectionType.power
Mstr.colorNormal = sm.color.new(0x14514ff)
Mstr.colorHighlight = sm.color.new(0x114514aa)

Mstr.colorWhite = "eeeeeeff"
Mstr.colorBlack = "222222ff"
Mstr.colorRed = "d02525ff"
Mstr.colorBlue = "0a3ee2ff"
Mstr.colorPurple = "7514edff"
Mstr.colorGreen = "19e753ff"
Mstr.colorLightBlue = "2ce6e6ff"
Mstr.colorYellow = "e2db13ff"
Mstr.logicgateName ={"and","or","xor","nand","nor","nxor"}

--server----------------------------------------------

function Mstr.server_onCreate(self)
...
function Mstr.server_onFixedUpdate(self,dt)
	...
	elseif self.dtTime <=0 then -- 尝试生成（恒为0,（-1000，0）是延迟缓冲层, 因为不确定有没有这个bug）
...
function Mstr.server_createMissile(self,data) -- return [error_level] [the_missile_itself]
	if self.saved then
...
function Mstr.server_FixedSaveMissile(self)
...
function Mstr.server_recycleScrap(self)
	local recycledCnt = self:server_deleteMissile()
	self.dtTime = self.coldTime - recycledCnt + 1
end

function Mstr.server_deleteMissile(self) -- return delete shape count
	local recycledCnt = 0
...
function Mstr.server_createPositionList(self) -- return a Table of missilePosition
	local oup = {}
...
function Mstr.server_clearSpace(self)
	local allShapes = self.shape.body:getCreationShapes()
	for i,Cshape in pairs(allShapes)do
...
function Mstr.client_sendMessage(self,data)
